...
|**Release Date**|**Note**|
| -------------------------| -----------------|
|xx/12/2017                |**Added**: <BR/> Contact search API: add working location in search param + returned value <BR/> Contact search API: allow search by company name <BR/> Application API: method to search by given job id and given application stage <BR/> Application API: method to search by given application stage <BR/> Application API: method to update application's substatus <BR/> Activity API: the search APIs return assignee information <BR/> **Fixed**: <BR/> Remove property floated_job in method to create and update job (the business is not yet supported through API)                          |
